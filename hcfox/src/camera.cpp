  ...
  , tag(0)
{
  ...
  m_time_sub = pnode.subscribe<sensor_msgs::TimeReference>(
    "trigger_time", 10, &Camera::time_callback, this,
    ros::TransportHints().tcpNoDelay());

  syncClient = pnode.serviceClient<dji_sdk::SetHardSync>("dji_sdk/set_hardsyc");

  dji_sdk::SetHardSync hc;
  hc.request.frequency = 0;
  hc.request.tag       = 0;
  if (syncClient.call(hc))
  {
    ROS_INFO("success");
  }
  else
  ...
  for (int i = 0; i < cam_cnt; i++)
  {
    ...
    std::cout << "serial: " << cs.serial << std::endl;
  }
    ...
    if (grab_image_data())
    {
      for (auto& item : img_publisher)
      {
        ...
        if (is_slave_mode())
        {
          ...
          {
            ros::spinOnce();
            if ((ros::Time::now() - n).toSec() > (1.0 / m_fps))
            {
              ROS_WARN("stamp overfolw");
              dji_sdk::SetHardSync hc;
              hc.request.frequency = m_fps;
              hc.request.tag       = tag++;
              if (syncClient.call(hc))
              {
                ROS_INFO("success");
          ...
          if (!timeQueue.empty())
          {
            img_buffer.at(serial).header.stamp = timeQueue.front().time_ref;
            timeQueue.pop();
        ...
        //        ROS_INFO("Duration %lf", (ros::Time::now() -
        //        capture_time).toSec());
      }
    }
    else
    {
      ROS_WARN("update SYNC");
      dji_sdk::SetHardSync hc;
      hc.request.frequency = m_fps;
      hc.request.tag       = tag++;
      if (!syncClient.call(hc))
...
Camera::send_software_request()
{
  ...
  usleep(5000); // necessary short sleep to warm up the camera
...
void
Camera::time_callback(const sensor_msgs::TimeReferenceConstPtr& pmsg)
{
  timeQueue.push(*pmsg);
}
}
